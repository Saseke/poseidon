...
public class ItemCatDirective implements TemplateDirectiveModel {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars,
		...
		if (!params.containsKey(LIMIT) || params.get(LIMIT) == null
				|| !params.containsKey(REMARK) || params.get(REMARK) == null) {
			throw new TemplateModelException("This is miss limit param");
		}
		LOGGER.info("the remark is {} ,limit is {}", params.get(REMARK).toString(),
				Integer.valueOf(params.get(LIMIT).toString()));
		List<ItemCatDto> itemCatDtos = itemCatService.getByRemark(
